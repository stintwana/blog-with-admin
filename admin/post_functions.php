...
function getAllPosts()
{
	...
	// Admin can view all posts\\\\AS p, users AS u WHERE p.user_id=u.id
...
function createPost($request_values)
	{
		...
		if (count($errors) == 0) {
			$query = "INSERT INTO posts (user_id, title, slug, image, body, published, created_at, updated_at) VALUES(".$_SESSION['user']['id'].", '$title', '$post_slug', '$featured_image', '$body', $published, now(), now())";
	...
	* * * * * * * * * * * * * * * * * * * * * */
	...
	{
		global $db, $title,$featured_image, $post_slug, $body, $published, $isEditingPost, $post_id;
